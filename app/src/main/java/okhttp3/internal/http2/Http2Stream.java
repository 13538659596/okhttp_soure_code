...
public final class Http2Stream {
  ...
  private final List<okhttp3.internal.http2.Header> requestHeaders;

  /** Response headers yet to be {@linkplain #takeResponseHeaders taken}. */
  private List<okhttp3.internal.http2.Header> responseHeaders;
  ...
  Http2Stream(int id, Http2Connection connection, boolean outFinished, boolean inFinished,
      List<okhttp3.internal.http2.Header> requestHeaders) {
  ...
  public List<okhttp3.internal.http2.Header> getRequestHeaders() {
   ...
   * delimited by 'null'.
   */
  public synchronized List<okhttp3.internal.http2.Header> takeResponseHeaders() throws IOException {
    ...
    List<okhttp3.internal.http2.Header> result = responseHeaders;
    if (result != null) {
   ...
   */
  public void sendResponseHeaders(List<okhttp3.internal.http2.Header> responseHeaders, boolean out) throws IOException {
   ...
   * transmitted.
   */
  public void close(okhttp3.internal.http2.ErrorCode rstStatusCode) throws IOException {
  ...
  void receiveHeaders(List<okhttp3.internal.http2.Header> headers) {
    assert (!Thread.holdsLock(Http2Stream.this));
    boolean open = true;
    synchronized (this) {
  ...
  private final class FramingSource implements Source {
    ...
    void receive(BufferedSource in, long byteCount) throws IOException {
      ...
      while (byteCount > 0) {
        ...
        if (flowControlError) {
          in.skip(byteCount);
          closeLater(okhttp3.internal.http2.ErrorCode.FLOW_CONTROL_ERROR);
          return;
        }
  ...
  void cancelStreamIfNecessary() throws IOException {
    ...
    if (cancel) {
      ...
      Http2Stream.this.close(okhttp3.internal.http2.ErrorCode.CANCEL);
    } else if (!open) {
      connection.removeStream(id);
    }
  }
