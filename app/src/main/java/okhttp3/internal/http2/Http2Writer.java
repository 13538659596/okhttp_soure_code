...
final class Http2Writer implements Closeable {
  ...
  Http2Writer(BufferedSink sink, boolean client) {
    ...
    this.hpackWriter = new okhttp3.internal.http2.Hpack.Writer(hpackBuffer);
  ...
  public synchronized void applyAndAckSettings(okhttp3.internal.http2.Settings peerSettings) throws IOException {
    if (closed) throw new IOException("closed");
  ...
  public synchronized void pushPromise(int streamId, int promisedStreamId,
      List<okhttp3.internal.http2.Header> requestHeaders) throws IOException {
    if (closed) throw new IOException("closed");
  ...
  public synchronized void synStream(boolean outFinished, int streamId,
      int associatedStreamId, List<okhttp3.internal.http2.Header> headerBlock) throws IOException {
    if (closed) throw new IOException("closed");
    headers(outFinished, streamId, headerBlock);
  }

  public synchronized void synReply(boolean outFinished, int streamId,
      List<okhttp3.internal.http2.Header> headerBlock) throws IOException {
    if (closed) throw new IOException("closed");
    headers(outFinished, streamId, headerBlock);
  }

  public synchronized void headers(int streamId, List<okhttp3.internal.http2.Header> headerBlock)
      throws IOException {
    if (closed) throw new IOException("closed");
    headers(false, streamId, headerBlock);
  }

  public synchronized void rstStream(int streamId, okhttp3.internal.http2.ErrorCode errorCode)
      throws IOException {
    if (closed) throw new IOException("closed");
  ...
  public synchronized void settings(okhttp3.internal.http2.Settings settings) throws IOException {
    if (closed) throw new IOException("closed");
