...
public final class Http2Codec implements HttpCodec {
  ...
  @Override public void writeRequestHeaders(Request request) throws IOException {
    ...
    List<okhttp3.internal.http2.Header> requestHeaders = http2HeadersList(request);
  ...
  @Override public Response.Builder readResponseHeaders(boolean expectContinue) throws IOException {
    List<okhttp3.internal.http2.Header> headers = stream.takeResponseHeaders();
  ...
  public static List<okhttp3.internal.http2.Header> http2HeadersList(Request request) {
    Headers headers = request.headers();
    List<okhttp3.internal.http2.Header> result = new ArrayList<>(headers.size() + 4);
    result.add(new okhttp3.internal.http2.Header(TARGET_METHOD, request.method()));
    result.add(new okhttp3.internal.http2.Header(TARGET_PATH, RequestLine.requestPath(request.url())));
    String host = request.header("Host");
    if (host != null) {
      result.add(new okhttp3.internal.http2.Header(TARGET_AUTHORITY, host)); // Optional.
    }
    result.add(new okhttp3.internal.http2.Header(TARGET_SCHEME, request.url().scheme()));

    for (int i = 0, size = headers.size(); i < size; i++) {
      // header names must be lowercase.
      ByteString name = ByteString.encodeUtf8(headers.name(i).toLowerCase(Locale.US));
      if (!HTTP_2_SKIPPED_REQUEST_HEADERS.contains(name)) {
        result.add(new okhttp3.internal.http2.Header(name, headers.value(i)));
      }
    }
    return result;
  }
  ...
  public static Response.Builder readHttp2HeadersList(List<okhttp3.internal.http2.Header> headerBlock) throws IOException {
