 ...
 *
 * <p>This implementation assumes we do not send an increased {@link okhttp3.internal.http2.Settings#getMaxFrameSize frame
 * size setting} to the peer. Hence, we expect all frames to have a max length of {@link
 * okhttp3.internal.http2.Http2#INITIAL_MAX_FRAME_SIZE}.
 */
final class Http2Reader implements Closeable {
  ...
  Http2Reader(BufferedSource source, boolean client) {
    ...
    this.hpackReader = new okhttp3.internal.http2.Hpack.Reader(4096, continuation);
  }
  ...
  public void readConnectionPreface(Handler handler) throws IOException {
    if (client) {
      ...
      if (logger.isLoggable(FINE)) logger.fine(String.format("<< CONNECTION %s", connectionPreface.hex()));
  ...
  private void readHeaders(Handler handler, int length, byte flags, int streamId)
    ...
    List<okhttp3.internal.http2.Header> headerBlock = readHeaderBlock(length, padding, flags, streamId);

    handler.headers(endStream, streamId, -1, headerBlock);
  }

  private List<okhttp3.internal.http2.Header> readHeaderBlock(int length, short padding, byte flags, int streamId)
      throws IOException {
  ...
  private void readRstStream(Handler handler, int length, byte flags, int streamId)
    ...
    okhttp3.internal.http2.ErrorCode errorCode = okhttp3.internal.http2.ErrorCode.fromHttp2(errorCodeInt);
    if (errorCode == null) {
   ...
   * continuation frames as they are needed by {@link okhttp3.internal.http2.Hpack.Reader#readHeaders()}.
   */
